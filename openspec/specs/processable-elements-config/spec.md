...
The text processor `shouldProcessNode()` SHALL return `false` for any text node whose ancestor chain does not contain at least one element whose tag name (case-insensitive) is in the `processableElements` list.

#### Scenario: Text inside a configured element is processed

- **WHEN** a text node's ancestor chain contains a `<p>` element and `'p'` is in `processableElements`
...
- **WHEN** a text node's ancestor chain contains only a `<div>` and `'div'` is not in `processableElements`
- **THEN** `shouldProcessNode()` SHALL return `false`

#### Scenario: Text inside nested configured element is processed

- **WHEN** a text node lives inside `<article><p>text</p></article>` and `'p'` is in `processableElements`
- **THEN** `shouldProcessNode()` SHALL return `true`

#### Scenario: Empty processable elements list blocks all processing

- **WHEN** `processableElements` is an empty array `[]`
